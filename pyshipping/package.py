...
def buendelung(kartons, maxweight=31000, maxgurtmass=3000):
    ...
    def buendelung_moeglich(box_a, box_b):
        ...
        if tmp.weight is not None and int(tmp.weight) > maxweight:
            return False
        elif tmp.gurtmass is not None and int(tmp.gurtmass) > maxgurtmass:
            return False
        return True
...
def pack_in_bins(kartons, versandkarton):
    ...
    Package('254x171x368'), Package('201x172x349'), Package('254x171x368')], Package('600x400x400'))
...
class PackageTests(unittest.TestCase):
    ...
    def test_sort(self):
        ...
        sorted_data = sorted(data, key=lambda package: package.volume, reverse=False)
